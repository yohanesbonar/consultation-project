...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		...
		const options = {
			method: 'GET',
			url: `${process.env.NEXT_PUBLIC_CONSUL_API_BASE_URL}v1/checkout/${id}/detail`,
			timeout: 60000,
			headers: {
				...(token ? { 'x-token': token } : {}),
			},
		...
		console.warn(
			'options for product detail transaction',
			JSON.stringify(req.query),
			req.headers.token,
		);
		logApiError('GET /v1/product/transaction/detail/[id]', error);
